...
import dev.jarcadia.redao.proxy.Internal;
...
public class ProxyMetadataFactory {
	...
	protected ProxyMetadata create(Class<? extends Proxy> proxyClass) {
		...
		*/
		...
		return new ProxyMetadata(proxyClass, daoGetter, reproxyMethod, defaultMethods, getters, setterFieldsMap, clearerFieldsMap, passthroughMethodsMap, lookup);
	}
	...
	private boolean isTypeGetter(Method method) {
		return "getType".equals(method.getName()) && String.class.equals(method.getReturnType()) && method.getParameterCount() == 0;
	}
	...
	private boolean isReproxyMethod(Method method) {
		return "as".equals(method.getName()) && Void.TYPE == method.getReturnType() && method.getParameterCount() == 1;
	}
